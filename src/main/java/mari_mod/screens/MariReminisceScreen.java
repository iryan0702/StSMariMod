...
public class MariReminisceScreen extends CardRewardScreen
{
    ...
    public void open(final ArrayList<AbstractCard> cards, final RewardItem rItem, final String header) {
        AbstractDungeon.cardRewardScreen = MariMod.mariReminisceScreen;
        this.codex = false;
        this.draft = false;
        this.rItem = rItem;
        this.onCardSelect = true;
        ...
        this.placeCards(Settings.WIDTH / 2.0f, MariReminisceScreen.CARD_TARGET_Y);
    }
    ...
    private void placeCards(final float x, final float y) {
        switch (this.rewardGroup.size()) {
            ...
            case 3: {
                this.rewardGroup.get(0).target_x = Settings.WIDTH / 2.0f - AbstractCard.IMG_WIDTH - MariReminisceScreen.PAD_X;
                this.rewardGroup.get(1).target_x = Settings.WIDTH / 2.0f;
                this.rewardGroup.get(2).target_x = Settings.WIDTH / 2.0f + AbstractCard.IMG_WIDTH + MariReminisceScreen.PAD_X;
                this.rewardGroup.get(0).target_y = y;
                this.rewardGroup.get(1).target_y = y;
            ...
            case 4: {
                this.rewardGroup.get(0).target_x = Settings.WIDTH / 2.0f - AbstractCard.IMG_WIDTH * 1.5f - MariReminisceScreen.PAD_X * 1.5f;
                this.rewardGroup.get(1).target_x = Settings.WIDTH / 2.0f - AbstractCard.IMG_WIDTH / 2.0f - MariReminisceScreen.PAD_X / 2.0f;
                this.rewardGroup.get(2).target_x = Settings.WIDTH / 2.0f + AbstractCard.IMG_WIDTH / 2.0f + MariReminisceScreen.PAD_X / 2.0f;
                this.rewardGroup.get(3).target_x = Settings.WIDTH / 2.0f + AbstractCard.IMG_WIDTH * 1.5f + MariReminisceScreen.PAD_X * 1.5f;
                this.rewardGroup.get(0).target_y = y;
                this.rewardGroup.get(1).target_y = y;
                this.rewardGroup.get(2).target_y = y;
    ...
    static {
        ...
        TEXT = MariReminisceScreen.uiStrings.TEXT;
