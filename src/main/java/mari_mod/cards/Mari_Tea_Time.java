...
public class Mari_Tea_Time extends AbstractMariCard {
    ...
    private static final int KINDLE_ENERGY_GAIN = 1;
    private static final int KINDLE_ENERGY_GAIN_UPGRADE = 1;
    ...
    public Mari_Tea_Time(){
        ...
        this.baseMagicNumber = KINDLE_ENERGY_GAIN;
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        AbstractDungeon.actionManager.addToBottom(new MariTeaTimeAction(ENERGY_GAIN));


        if(target.hasPower(Radiance_Power.POWER_ID) && target.getPower(Radiance_Power.POWER_ID).amount >= 1){
            this.successfulKindle(target);
        }
        AbstractDungeon.actionManager.addToBottom(new MariSuccessfulKindleAction(target, new GainEnergyAction(this.magicNumber)));
    }
    ...
    public void upgrade() {
        if (!this.upgraded) {
            ...
            upgradeName();
            upgradeMagicNumber(KINDLE_ENERGY_GAIN_UPGRADE);
        }
    }
}