...
public class Mari_Oh_My_God extends AbstractMariCard {
    ...
    private static final int PLAY_CARDS = 4;
    private static final int UPGRADE_PLAY_CARDS = 6;
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        AbstractDungeon.actionManager.addToBottom(new MariOhMyGodAction(this.magicNumber, m, EXTENDED_DESCRIPTION[0]));
    }

    public void upgrade() {
        if (!this.upgraded) {
            this.upgradeMagicNumber(UPGRADE_PLAY_CARDS);
            upgradeName();
        }
    }
}