...
public class MariUncommonReward extends RewardItem {
    private float REWARD_TEXT_X = 833.0F * Settings.scale;
    private ArrayList<AbstractGameEffect> effects = new ArrayList<>();

    public MariUncommonReward(AbstractCard.CardColor colorType) {
        ...
        for(int i = 0; i < cardsToJankReplace; i++){
            AbstractCard cardToAdd = AbstractDungeon.getCard(AbstractCard.CardRarity.UNCOMMON).makeCopy();
            while(cardListDuplicate(cardToAdd)){
                cardToAdd = AbstractDungeon.getCard(AbstractCard.CardRarity.UNCOMMON).makeCopy();
            }
